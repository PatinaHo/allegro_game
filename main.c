...
void game_update(void) {
    if (active_scene == SCENE_START) {
        ...
        // Uncomment and fill in the code below.
        if (plane.x < al_get_bitmap_width(start_img_plane)/2)
            plane.x = al_get_bitmap_width(start_img_plane)/2;
        else if (plane.x > SCREEN_W - (al_get_bitmap_width(start_img_plane)/2))
            plane.x = SCREEN_W - (al_get_bitmap_width(start_img_plane)/2);
        if (plane.y < al_get_bitmap_height(start_img_plane)/2)
            plane.y = al_get_bitmap_height(start_img_plane)/2;
        else if (plane.y > SCREEN_H - (al_get_bitmap_height(start_img_plane)/2))
            plane.y = SCREEN_H - (al_get_bitmap_height(start_img_plane)/2);
        // [HACKATHON 2-6]
...
void game_draw(void) {
    if (active_scene == SCENE_MENU) {
        ...
        if (pnt_in_rect(mouse_x, mouse_y, SCREEN_W - 48, 10, 38, 38)){
            game_log("%d", pnt_in_rect(mouse_x, mouse_y, SCREEN_W - 48, 10, 38, 38));
...
//            al_draw_bitmap(img_bullet, bullets[i].x, bullets[i].y, 0);
